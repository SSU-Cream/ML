...
def train(run_name, speaker, start_epoch, stop_epoch, img_c, img_w, img_h, frames_n, absolute_max_string_len, minibatch_size):
    ...
    lipnet.model.fit_generator(generator=lip_gen.next_train(),
                        ...
                        pickle_safe=True)
    print("\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Train End >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n")
    # lipnet.model.save('/home/sun/LipNet/LipNet/training/overlapped_speakers/s1/results/xfullmodel_test/my_model.h5')

if __name__ == '__main__':
    run_name = datetime.datetime.now().strftime('%Y:%m:%d:%H:%M:%S') # now time ex)2019:05:15:16:14:20
    run_name = "final_test" # train name
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Training name : " + run_name)
    speaker = sys.argv[1] # speaker : s{1}
    train(run_name, speaker, 20, 21, 3, 100, 50, 75, 2, 2) #150 epoch color 100x50 75frames 32len string minibatch_size 50 -> 2len
    #train(run_name, speaker, 0, 5000, 3, 100, 50, 75, 32, 2) #5000 epoch color 100x50 75frames 32len string minibatch_size 50
    # train(run_name, speaker, 0, 5000, 3, 100, 50, 75, 32, 50) #5000 epoch color 100x50 75frames 32len string minibatch_50 size
